...
//Дамб-компонент для редера сетки на 32 человека

const Elimination32 = ({eliminations}) => {
    ...
    const round4AArray = transformFights(eliminations.rounds[4].fights.slice(0,7), fighters);
    const round4BArray = transformFights(eliminations.rounds[4].fights.slice(6,15), fighters);

    const round4A = renderEliminationsRounds(round4AArray);
    const round4B = renderEliminationsRounds(round4BArray);
